...
TextFilterWidget = (function ($) {
    ...
    */
    textFilterWidget.prototype.onRender = function (container, lang, typeName, values, cb) {
        ...
        this.lang = lang;
        this.value = values.length > 0 ? values[0] : { filterType: 2, filterValue: "" };//support only one filter value (changed from 1 to 2, so contains is the default)
        this.renderWidget();
        this.registerEvents();
    };
    /***
...
NumberFilterWidget = (function ($) {
    ...
    numberFilterWidget.prototype.renderWidget = function () {
        var html = '<div class="form-group">\
                        <label>' + this.lang.filterTypeLabel + '</label>\
                        <select class="grid-filter-type form-control">\
                            <option value="1" ' + (this.value.filterType == "2" ? "selected=\"selected\"" : "") + '>' + this.lang.filterSelectTypes.Equals + '</option>\
...
DateTimeFilterWidget = (function ($) {
    ...
    dateTimeFilterWidget.prototype.onRender = function (container, lang, typeName, values, applycb, data) {
        ...
        this.container = container;
        this.lang = lang;
        this.value = values.length > 0 ? values[0] : { filterType: 1, filterValue: "" };//support only one filter value  (changed from 1 to 2, so contains is the default)
        this.renderWidget();
        this.registerEvents();
    };
...
BooleanFilterWidget = (function ($) {
    ...
    booleanFilterWidget.prototype.registerEvents = function () {
        ...
        applyBtn.click(function () {
            var filterValues = [{ filterType: "2", filterValue: $(this).attr("data-value") }]; //(changed from 1 to 2, so contains is the default)
            $context.cb(filterValues);
        });
    };
